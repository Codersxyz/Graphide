...
def pie_chart_gui(window) :
    ...
    drop_menu = tk.OptionMenu(window, clicked, *options)
    drop_menu.config(width=31,height=2,border=0.5,font=('Times New Roman',15,'normal'))
    drop_menu.grid (row=0,column=0,pady=2,padx=0,sticky='nw')
...
# Buttons to add more entries
    ...
    plot = tk.Button(master=left_frame,text='Draw',width=49,height=3,
                     command=lambda:pie_chart_fun.pie_chart(canvas,label_entry_list,value_entry_list))
